...
export const getOutput = (
  ...
  switch (format) {
    ...
    case "markdown":
      // This app never requests "markdown" output (see use-minimal-tiptap.ts's
      // createExtensions note) - getMarkdown only exists when the Markdown extension is
      // loaded, which it isn't here, so this branch is unreachable in practice. Cast
      // rather than widen the Editor type for a codepath we don't use.
      return editor.isEmpty ? "" : ((editor as unknown as { getMarkdown: () => string }).getMarkdown?.() ?? "")
    default:
