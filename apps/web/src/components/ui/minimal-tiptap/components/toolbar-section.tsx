  ...
  return (
    <>
      ...
      {dropdownActions.length > 0 && (
        <DropdownMenu>
          {/* No `tooltip` prop passed here (unlike the plain buttons above) - see
              toolbar-button.tsx's note on why stacking a Tooltip inside another Base UI
              trigger's `render` breaks it. aria-label still carries the accessible name. */}
          <DropdownMenuTrigger tabIndex={-1} render={<ToolbarButton isActive={isDropdownActive} aria-label={dropdownTooltip} className={cn("gap-0", dropdownClassName)} size={size} variant={variant} />}>{dropdownIcon || <CaretDownIcon className="size-4" />}</DropdownMenuTrigger>
          {/* Explicit width, not "w-full" (the upstream registry's original value): this
              menu renders in a portal via Base UI's fixed positioning, so "w-full" resolves
              against the viewport rather than the small toolbar button anchoring it - a
              full-width dropdown, not a menu sized to its own items. */}
