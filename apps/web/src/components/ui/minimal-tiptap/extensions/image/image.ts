...
const handleDataUrl = (src: string): { blob: Blob; extension: string } => {
  // A well-formed data URL always has this shape ("data:<mime>;base64,<data>") - the
  // fallbacks below are only to satisfy noUncheckedIndexedAccess on the split results,
  // not because a malformed URL is actually expected here.
